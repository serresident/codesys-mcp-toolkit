...
export async function startMcpServer(config: ServerConfig) {
    ...
    // --- Helper Function (resolveProjectPath) ---
    function resolveProjectPath(providedPath: string, workspaceDir: string): string {
            ...
            if (fs.existsSync(absPath)) {
                ...
                if (stats.isDirectory()) {
                    ...
                    if (projectFiles.length > 0) {
                        console.error(`SERVER.TS: Resolved directory ${absPath} to latest project: ${projectFiles[0].path}`);
                        return projectFiles[0].path;
                    }
                }
            }
        } catch (e: any) {
            console.error(`SERVER.TS: Error resolving project path: ${e.message}`);
        }
        return absPath;
    }
...
# Global config variables will be replaced by Node.js server
...
def get_extension(node):
    ...
    if type_str == POU_GUID:
        try:
            ...
            # Fallback: parse textual_declaration
            decl = ""
            ...
            if 'FUNCTION_BLOCK' in words:
                ...
                return '.prg.st'
            return '.prg.st' # default fallback
...
`;
    ...
    server.resource("project-structure", projectStructureTemplate, async (uri, params) => {
        ...
        try {
            const absoluteProjPath = resolveProjectPath(projectPath, WORKSPACE_DIR);
    ...
    server.tool(
        ...
        async (args) => { // Handler
            const { filePath } = args;
            let absPath = resolveProjectPath(filePath, WORKSPACE_DIR);
    ...
    server.tool(
        ...
        async (args) => { // Handler
            ...
            let absPath = resolveProjectPath(projectFilePath, WORKSPACE_DIR);
    ...
    server.tool(
        ...
        "Exports CODESYS project code and folders to disk as Structured Text (.st) files and XML.",
        {
            projectFilePath: z.string().describe("Path to the CODESYS project file or directory (e.g. 'C:/Projects/MyPLC.project' or 'D:/Projects/CEX_15/апп 511/PLC')."),
            exportDirPath: z.string().describe("Path to the folder on disk where code should be exported.")
        },
        async (args) => {
            const { projectFilePath, exportDirPath } = args;
            let absProjPath = resolveProjectPath(projectFilePath, WORKSPACE_DIR);
            let absExportPath = path.normalize(path.isAbsolute(exportDirPath) ? exportDirPath : path.join(WORKSPACE_DIR, exportDirPath));
            console.error(`Tool call: export_project_sources: Project='${absProjPath}', ExportDir='${absExportPath}'`);
            try {
                const escProjPath = absProjPath.replace(/\\/g, '\\\\');
                const escExportPath = absExportPath.replace(/\\/g, '\\\\');
                let script = EXPORT_SOURCES_SCRIPT_TEMPLATE
                    .replace("{PROJECT_FILE_PATH}", escProjPath)
                    .replace("{EXPORT_DIR_PATH}", escExportPath);
                
                const result = await executeCodesysScript(script, codesysExePath, codesysProfileName);
                const success = result.success && result.output.includes("SCRIPT_SUCCESS");
                return { content: [{ type: "text", text: success ? `Sources exported successfully to: ${absExportPath}` : `Failed to export sources. Output:\n${result.output}` }], isError: !success };
            } catch (e: any) {
                console.error(`Error export_project_sources: ${e.message}`);
                return { content: [{ type: "text", text: `Error: ${e.message}` }], isError: true };
            }
        }
    );
    ...
    server.tool(
        ...
        "Imports Structured Text (.st) files from disk back into the CODESYS project, updating or creating objects natively.",
        {
            projectFilePath: z.string().describe("Path to the CODESYS project file or directory (e.g. 'C:/Projects/MyPLC.project' or 'D:/Projects/CEX_15/апп 511/PLC')."),
            importDirPath: z.string().describe("Path to the folder on disk from where code should be imported.")
        },
        async (args) => {
            const { projectFilePath, importDirPath } = args;
            let absProjPath = resolveProjectPath(projectFilePath, WORKSPACE_DIR);
            let absImportPath = path.normalize(path.isAbsolute(importDirPath) ? importDirPath : path.join(WORKSPACE_DIR, importDirPath));
            console.error(`Tool call: import_project_sources: Project='${absProjPath}', ImportDir='${absImportPath}'`);
            try {
                const escProjPath = absProjPath.replace(/\\/g, '\\\\');
                const escImportPath = absImportPath.replace(/\\/g, '\\\\');
                let script = IMPORT_SOURCES_SCRIPT_TEMPLATE
                    .replace("{PROJECT_FILE_PATH}", escProjPath)
                    .replace("{IMPORT_DIR_PATH}", escImportPath);
                
                const result = await executeCodesysScript(script, codesysExePath, codesysProfileName);
                const success = result.success && result.output.includes("SCRIPT_SUCCESS");
                return { content: [{ type: "text", text: success ? `Sources imported successfully from: ${absImportPath}` : `Failed to import sources. Output:\n${result.output}` }], isError: !success };
            } catch (e: any) {
                console.error(`Error import_project_sources: ${e.message}`);
                return { content: [{ type: "text", text: `Error: ${e.message}` }], isError: true };
            }
        }
    );
