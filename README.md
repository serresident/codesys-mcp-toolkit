...
### 🛠️ Ручной режим и VS Code Tasks (Синхронизация по команде)

Если вы хотите запускать импорт или экспорт вручную (без участия ИИ или когда MCP-сервер остановлен), вы можете воспользоваться тремя инструментами:

#### 1. Интерактивные VS Code Tasks (Встроенные задачи)
Мы настроили интерактивные задачи для VS Code. Вы можете запустить их через меню `Terminal -> Run Task...` (или нажав `Ctrl+Shift+P` -> `Tasks: Run Task`):
*   **`CODESYS: Export sources (Выгрузить в .st)`** — выгрузка проекта по путям по умолчанию.
*   **`CODESYS: Import sources (Залить код в CODESYS)`** — импорт изменений по путям по умолчанию.
*   **`CODESYS: Export sources [Custom Paths]`** — запрашивает пути к проекту и папке выгрузки через интерактивные поля ввода в VS Code.
*   **`CODESYS: Import sources [Custom Paths]`** — запрашивает пути для импорта измененного кода обратно с помощью полей ввода в VS Code.

#### 2. Standalone GUI (Графический интерфейс)
В корне репозитория находится Python-скрипт `manager.py`. Вы можете запустить его, дважды щелкнув по файлу `manage.bat` или выполнив команду в терминале:
```bash
python manager.py
```
В открывшемся окне в нижней части (**"Синхронизация Git (Ручной режим)"**):
*   Нажмите на кнопку с тремя точками `...` рядом с полями путей, чтобы интерактивно выбрать файл/директорию проекта и папку для `.st` файлов.
*   Нажмите **"Экспорт исходников"** для выгрузки.
*   Нажмите **"Импорт исходников"** для загрузки изменений обратно в CODESYS.
Прогресс операции отобразится в отдельном открывшемся окне логов.

#### 3. CLI консоль (Прямой запуск)
Вы также можете запускать скрипт напрямую через Node.js с указанием аргументов:
```bash
# Выгрузка (Экспорт)
node sync_cli.js export "D:\Projects\CEX_15\апп 511\PLC" "D:\Projects\CEX_15\апп 511\PLC\project_sources"

# Загрузка (Импорт)
node sync_cli.js import "D:\Projects\CEX_15\апп 511\PLC" "D:\Projects\CEX_15\апп 511\PLC\project_sources"
```
